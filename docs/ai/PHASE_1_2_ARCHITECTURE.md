...
3. The chat service sends the entire planner list to IAiToolExecutor for preflight; the shared `AiPlannerPolicy` permits exactly seven read tools, and more than three calls, unknown/versioned tools or malformed arguments fail closed before any handler runs.
...
AiPendingToolAction is a short-lived server record with action/user/session/tool/version/request hash/resource/normalized allowlisted arguments/expiry/state/lease/attempt/error/confirmation/execution/cancellation/idempotency hash/result reference and a server-derived confirmation token. The token is immutable, required, compared in constant time, and is derived from the binding fields rather than exposing provider-specific SQL `rowversion`; this behaves consistently on SQL Server and SQLite. Raw prompts, model output, passwords, tokens and full medical text are not stored. Cancellation/reschedule preparation uses a 12-minute TTL. Execution:
